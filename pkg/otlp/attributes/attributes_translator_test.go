...
func TestInternalTelemetryMetrics(t *testing.T) {
	...
	mWithHostname := pcommon.NewMap()
	mWithHostname.FromRaw(map[string]any{
		"datadog.host.name": "testhost",
	})

	mWithoutHostname := pcommon.NewMap()

	attributeSet := attribute.NewSet(attribute.String("signal", "test"))

	_, _ = translator.MapToSource(context.Background(), mWithHostname, attributeSet)
	_, _ = translator.MapToSource(context.Background(), mWithoutHostname, attributeSet)
	_, _ = translator.MapToSource(context.Background(), mWithoutHostname, attributeSet)
