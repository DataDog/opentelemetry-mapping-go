...
func TestProcessExtractTags(t *testing.T) {
	...
	assert.Equal(t, []string{
		fmt.Sprintf("%s:%s", string(semconv16.ProcessExecutableNameKey), "otelcol"),
	}, pattrs.extractTags())
	...
	assert.Equal(t, []string{
		fmt.Sprintf("%s:%s", string(semconv16.ProcessExecutablePathKey), "/usr/bin/cmd/otelcol"),
	}, pattrs.extractTags())
	...
	assert.Equal(t, []string{
		fmt.Sprintf("%s:%s", string(semconv16.ProcessCommandKey), "cmd/otelcol"),
	}, pattrs.extractTags())
	...
	assert.Equal(t, []string{
		fmt.Sprintf("%s:%s", string(semconv16.ProcessCommandLineKey), "cmd/otelcol --config=\"/path/to/config.yaml\""),
	}, pattrs.extractTags())
}
